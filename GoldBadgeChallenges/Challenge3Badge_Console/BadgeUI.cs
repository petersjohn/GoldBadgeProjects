...
namespace Challenge3Badge_Console
{
    class BadgeUI
    {
        ...
        private void CreateNewBadge()
        {
            ...
            {
                Console.WriteLine("Badge number already exists.");
                PressAnyKeyToContinue();
                return;
            }
            DisplayDoors();
        ...
        {
            if (NoDoorsToRemove(badgeID))
            {
                Console.WriteLine("There are no doors to remove.");
                AnyKey();
                return;
            }
            ...
            if (_repo.RemoveDoorsFromBadge(badgeID, doorsToRemove) &&
            _repo.UpdateBadgeDictionary(badgeID, badge))
            {
                Console.Clear();
                Console.WriteLine("Door(s) removed successfully");
            }
            else
            {
                Console.Clear();
                Console.WriteLine("Error removing badge doors.");
            }
            AnyKey();
        ...
        private void AddDoorToBadge(int badgeID)
        {
            ...
            if (badge.Doors == null)
            {
                nullList = _repo.AddBadgesToNullList(badgeID, doorsToAdd);
                addStat = true;
            }
            else
            {
                addStat = _repo.AddNewDoorsToBadge(badgeID, doorsToAdd);
            }
            Console.Clear();
            if (addStat && _repo.UpdateBadgeDictionary(badgeID, badge))
            {
                Console.WriteLine("New Door(s) Added Successfuly.");
            }
            else
            {
                Console.WriteLine("Error adding new doors.");
            }
            AnyKey();
        ...
        private void RemoveAllDoors(int badgeID)
        {
            if (NoDoorsToRemove(badgeID))
            {
                Console.WriteLine("There are no doors to remove.");
            ...
            if (_repo.RemoveAllDoorsFromBadge(badgeID) &&
            _repo.UpdateBadgeDictionary(badgeID, badge))
            {
            ...
            {
                Console.WriteLine("Error removing all doors");
            }
        ...
        private bool NoDoorsToRemove(int badgeID)
        {
           
            if (_repo.GetBadgeByBadgeID(badgeID).Doors == null)
            {
                return true;
